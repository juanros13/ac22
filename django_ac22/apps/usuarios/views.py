...
def cambiar_obra_view(request, id_obra):
    ...
    if login_form.is_valid():
      ...
      if usuario is not None:
        if usuario.is_active:
          obras = Obra.objects.values_list('nombre', 'id').all()
          login(request, usuario)
          request.session['obras_disponibles'] = []
          request.session['obra_actual'] = obras.first()
          for obra in obras:
            sessionlist = request.session['obras_disponibles']
...
def olvide_mi_clave_enviar_email(request):
  ...
  if email:
    try:
      ...
      subject, from_email, to = 'KRAKENS SOFT - Contraseña acceso al sistema ', 'lobo022000@gmail.com', [email]
